...
func NewCommandRegistry(name string, description string, version string, logger *logrus.Logger) *CommandRegistry {
	root := &cobra.Command{
		Use:     name,
		Version: version,
		Short:   description,
	...
	root.Flags().BoolP("version", "V", false, "Print the version number of LazyFile")
	return newRegistry
}
